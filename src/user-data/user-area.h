...
struct UserAreaActionSets {
    ...
    static inline std::vector<UserAreaActionSets> fromJsonList(const json_view& jsonData) {
...
struct UserAreaItems {
    ...
    static inline std::vector<UserAreaItems> fromJsonList(const json_view& jsonData) {
...
struct UserAreaStatus {
    ...
    static inline UserAreaStatus fromJson(const json_view& jsonData) {
...
struct UserArea {
    ...
    static inline std::vector<UserArea> fromJsonList(const json_view& jsonData) {
        std::vector<UserArea> userAreas;
        for (const auto& item : jsonData) {
            UserArea userArea;
            userArea.areaId = item.value("areaId", 0);
            userArea.actionSets = UserAreaActionSets::fromJsonList(item.value("actionSets", json_view::array()));
            userArea.areaItems = UserAreaItems::fromJsonList(item.value("areaItems", json_view::array()));
            userArea.userAreaStatus = UserAreaStatus::fromJson(item.value("userAreaStatus", json_view()));
